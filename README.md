...
### Known limitation: long-brake cross-rate position and stop-time variation

This C2 suite is a regression guard, not proof of numerical convergence or physical fidelity, and the finer tested timestep is a finer-step comparison reference, not ground truth. Long braking currently shows known timestep sensitivity: a final-position (stopping-distance) spread of about `1.41 m` and a first-stop-time spread of about `0.475 s` across `60`/`120`/`240`/`480` Hz. The staged ABS approximation is sensitive to outer-step timing and to the discretely refreshed wheel rotational and traction state, and the low-speed stopped gate is refreshed at the outer-step rate, so rates stop at different positions and times.

Current behavior is intentionally retained. Tested narrow interventions — wheel-only substepping; ABS advancement using frozen slip; broad coupled longitudinal substepping; end-of-step slip refresh/reordering; fresh complete ABS traction state; and fresh stopped-state gating — each produced cross-scenario tradeoffs or worse coarse-rate wheel slip / excessive ABS modulation, with no clearly superior production behavior. These are rejected for the current model and evidence, not permanently forbidden architectures.

Future ABS or wheel/traction work should be revisited only alongside a more foundational wheel/traction improvement or a clearly superior control design. The regression ceilings (`MAXIMUM_LONG_BRAKE_POSITION_SPREAD_METERS = 1.6` and `STOP_TIME_SPREAD_CEILING_SECONDS = 0.60`) detect material worsening of these spreads; they do not establish that the current behavior is physically correct.
